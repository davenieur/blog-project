...
const getPostsByAuthor= async (slug, offset = 0, locale = 'es', altLocale = "en-US", limit) => {
  const postQuery = `query{
    siteCollection {
      items {
        postsCollection(limit: ${ limit }, order: [creationDate_DESC], skip: ${ offset }, locale: "${ locale }", where: { author: { slug: "${ slug }", sys:{id_exists:true} } } ) {
          items {
            title
            slug
